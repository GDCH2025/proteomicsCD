...
def Multimachine_with_cv(data, estimator, label, iter, n_repeats,features_subset):

    column_name = [label, "Region_code"]+features_subset
...
estimator_list = ['lgbm']
n_repeats=10
features = ['CD274', 'CHI3L1', 'REG1B', 'ITGAV', 'PRSS8', 'ITGA11', 'GDF15', 'DEFA1_DEFA1B', 'IL6', 'VWA1']

for lab in label:
    for estimator in estimator_list:
        for i in range(len(features)):
            features_subset = features[:i+1]    
            avg_res = Multimachine_with_cv(data, estimator, lab, iter, n_repeats,features_subset)
            avg_result = pd.concat([avg_result, avg_res], ignore_index=True)

avg_result.to_csv(f'/GDCH/proteinAI/result1/35protein_CD_accumulate_average_repeat10.csv', index=False)
