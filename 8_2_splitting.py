...
def Multimachine_with_cv(data, estimator, label, iter, n_repeats, model_name, model):
    ...
    for repeat in range(n_repeats):
        ...
        model_filename = f"/GDCH/proteinAI/model1/{model_name}_{label}_{estimator}_repeat{repeat + 1}_8_2_splitting.joblib"
    ...
    results_df.to_csv(f'/GDCH/proteinAI/result1/{model_name}_{label}_{estimator}_repeat10_8_2_splitting.csv', index=False)
...
data = pd.read_csv('/GDCH/proteinAI/data/revise/CDallpredictor_8_2_splitting.csv', sep=",")
iter = 100
label = ["CD"]
...
avg_result.to_csv(f'/GDCH/proteinAI/result1/allmodel_CD_average_repeat10_8_2_splitting.csv', index=False)
