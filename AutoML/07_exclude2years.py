...
def Multimachine_with_cv(data, estimator, label, iter, n_repeats, model_name, model):
    ...
    for repeat in range(n_repeats):
        ...
        model_filename = f"/GDCH/proteinAI/model1/{model_name}_{label}_{estimator}_repeat{repeat + 1}_exclude2years.joblib"
    ...
    results_df.to_csv(f'/GDCH/proteinAI/result1/{model_name}_{label}_{estimator}_repeat10_exclude2years.csv', index=False)
...
avg_result.to_csv(f'/GDCH/proteinAI/result1/allmodel_CD_average_repeat10_exclude2years.csv', index=False)
