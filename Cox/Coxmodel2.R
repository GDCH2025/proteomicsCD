...
for (i in 3:2739) {
  ...
  catvars<-c("sex","activity","Ethnicity","smoking","alcohol","healthy_diet","depression","anxiety","antibiotic","NSAID")
  alldata[catvars] <- lapply(alldata[catvars], factor)
  
  tryCatch({
    mycox <- coxph(Surv(time, CD== 1) ~ CDallpredictor_train[[i]]+age+sex+Ethnicity+TDindex+BMI+activity+smoking+alcohol+healthy_diet+depression+anxiety+antibiotic+NSAID, data = alldata)
...
write_csv(cumulative_cox, file = "/GDCH/proteinAI/result1/cox/CDmodel2cox.csv")
